<?php
  ...
  function makeWaveform($mp3File, $waveformFile = null) {
    ...
    if ($stereo) {
      ...
      exec("lame {$tmpFile} --scale-r 0.1 -m m -S -f -b 16 --resample 8 {$tmpname}.mp3 && lame -S --decode {$tmpname}.mp3 {$tmpname}_l.wav");
			// same as above, left channel
      exec("lame {$tmpFile} --scale-l 0.1 -m m -S -f -b 16 --resample 8 {$tmpname}.mp3 && lame -S --decode {$tmpname}.mp3 {$tmpname}_r.wav");
    ...
    if ($width) {
      ...
      imagepng($rimg, $waveformFile);
      imagedestroy($rimg);
    } else {
      imagepng($img, $waveformFile);
    }
